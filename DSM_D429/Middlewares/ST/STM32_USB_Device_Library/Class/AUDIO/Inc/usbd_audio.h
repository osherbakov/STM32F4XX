...
// #define AUDIO_IN_STREAMING_CTRL                       0x05
...
#define AUDIO_OUT_PACKET_NUM                          10
#define AUDIO_OUT_ADJ_SIZE														(2 * 2)
...
typedef struct
{
	// ------------ OUT Buffer - to receive samples from the host
  __IO uint32_t             out_alt_setting;
	uint32_t									out_start_play;
  uint8_t                   out_buffer[AUDIO_TOTAL_BUF_SIZE + AUDIO_OUT_ADJ_SIZE];
  uint16_t                  out_wr_ptr;
	// ------------ IN Buffer - to send samples to the host
  __IO uint32_t             in_alt_setting;
  uint8_t                   in_buffer[AUDIO_TOTAL_BUF_SIZE + AUDIO_OUT_ADJ_SIZE];
  uint16_t                  in_rd_ptr;
  uint16_t                  in_wr_ptr;
