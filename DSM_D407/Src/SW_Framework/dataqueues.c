...
uint32_t DataConvertSrc(void *pDst, uint32_t DstType, uint32_t DstChMask, void *pSrc, uint32_t SrcType, uint32_t SrcChMask, uint32_t nSrcBytes)
{
	...
	while(1)
	{
		for (dstIdx = 0; dstIdx < dstCntr; dstIdx++)
		{
			...
			nConsumedBytes +=  srcStep;
			if(nConsumedBytes >= nSrcBytes) return nGeneratedBytes;
...
uint32_t DataConvertDst(void *pDst, uint32_t DstType, uint32_t DstChMask, void *pSrc, uint32_t SrcType, uint32_t SrcChMask, uint32_t nDstBytes)
{
	int  srcStep, dstStep;
	int  srcSize, dstSize;
	int  srcChan, dstChan;
	...
	if((DstType == SrcType) && (DstChMask == SrcChMask))
	{
		memcpy(pDst, pSrc, nDstBytes);
		nGeneratedBytes = nDstBytes;
		return nGeneratedBytes;
	}		
	...
	while(1)
	{
		for (dstIdx = 0; dstIdx < dstCntr; dstIdx++)
		{
			...
			nGeneratedBytes += dstStep;
			nConsumedBytes +=  srcStep;
			if(nGeneratedBytes >= nDstBytes) return nConsumedBytes;
...
uint32_t DataConvertDstInfo(uint32_t DstType, uint32_t DstChMask, uint32_t SrcType, uint32_t SrcChMask, uint32_t nDstBytes)
{
	int  srcStep, dstStep;
	int  srcSize, dstSize;
	int  srcChan, dstChan;
	...
	DstChMask = (DstChMask == DATA_CHANNEL_ANY)? DATA_CHANNEL_ALL : DstChMask;
	DstChMask &= ((1 << dstChan) - 1);
	for(dstCntr = 0, Idx = 0; Idx < dstChan; Idx++)
	{
		if(DstChMask & 0x01) dstCntr++;
		DstChMask >>= 1;
	}
	...
	if(SrcChMask == DATA_CHANNEL_ANY)
	{
		...
		SrcChMask &= ((1 << srcChan) - 1);
		for(srcCntr = 0, Idx = 0; Idx < srcChan; Idx++)
		{
			if(SrcChMask & 0x01) srcCntr++;
			SrcChMask >>= 1;
		}
	}
