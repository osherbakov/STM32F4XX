...
int16_t inputw[LPC_FRAME] CCMRAM;
...
//	}
	...
	if (L_temp){
		norm_var = 4 -  norm_l(L_temp)/2;
		if (norm_var < 0)	norm_var = 0;
	} else
		norm_var = 0;
...
//			L_temp = L_mac(L_temp, inputw[i], inputw[i - j]);
		L_temp = L_v_inner(&inputw[j], &inputw[0], npts-j, 0,0, 1);
		L_temp = L_shl(L_temp, norm_var);
...
int16_t lpc_schur_q(int16_t autocorr[], int16_t lpc[], int16_t order)
{
	...
	y2_[0] = L_deposit_h(autocorr[1]);
	y2_[1] = L_add(L_deposit_h(autocorr[0]), L_mult(refc[0], autocorr[1]));

	for (i = 1; i < order; i++){
		y1_[0] = L_deposit_h(autocorr[i + 1]);
		L_temp = L_deposit_h(autocorr[i + 1]);

		for (j = 0; j < i; j++){
			y1_[j + 1] = L_add(y2_[j], L_mpy_ls(L_temp, refc[j]));
			L_temp = L_add(L_temp, L_mpy_ls(y2_[j], refc[j]));
		}
		...
		temp2 = abs_s(extract_h(L_shl(y2_[i], shift)));
		...
		y2_[i + 1] = L_add(y2_[i], L_mpy_ls(L_temp, refc[i]));
		L_v_equ(y2_, y1_, (int16_t) (i + 1));
	}
...
int16_t lpc_pred2lsp_q(int16_t lpc[], int16_t lsf[], int16_t order)
{
	...
	for (i = 0; i <= p2; i++){
		p_cof[i] = round_l(L_p_cof[i]);                         /* p_cof in Q10 */
		q_cof[i] = round_l(L_q_cof[i]);                         /* q_cof in Q10 */
	}
