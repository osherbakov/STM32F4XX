...
int16_t *vq_lspw_q(int16_t weight[], int16_t lsp[], int16_t lpc[],
				   int16_t order)
{
	register int16_t	i;
	int32_t	L_temp;
	...
	for (i = 0; i < order; i++){
		L_temp = lpc_aejw_q(lpc, lsp[i], order);               /* L_temp in Q19 */
				 ...
				 int16_t *a_indices, int16_t max_inner)
{
	...
	int16_t	tmp, uhatw_sq;
	int16_t	d_cj, d_opt;
	int16_t	*p_d, *n_d, *p_distortion;
	int16_t	*p_errors, *n_errors, *p_e;
...
int32_t vq_enc_q(const int16_t codebook[], int16_t u[], int16_t levels,
...
/*       w_fs - Q14, pitch - Q9                                               */
/* ========================================================================== */
void vq_fsw_q(int16_t w_fs[], int16_t num_harm, int16_t pitch)
{
	register int16_t	i;
