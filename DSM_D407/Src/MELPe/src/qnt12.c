...
void pitch_vq(struct melp_param *par)
{
	...
	if (cnt == 0){
		...
		for (i = 0; i < NF; i++){
			if (!par[i].uv_flag){
				quant_u_q(&target[i], &(quant_par.pitch_index), PIT_QLO_Q12,
						PIT_QUP_Q12, PIT_QLEV_M1, PIT_QLEV_M1_Q8, TRUE, 7);
				quant_u_dec_q(quant_par.pitch_index, &par[i].pitch,
...
void quant_bp(struct melp_param *par, int16_t num_frames)
{
	...
	for (i = 0; i < num_frames; i++){
		par[i].uv_flag = q_bpvc_q(par[i].bpvc, &(quant_par.bpvc_index[i]),
								NUM_BANDS);
					  ...
					  int16_t dim, BOOLEAN flag)
{
	...
	*==================================================================*/
	...
	for (s1 = 0; s1 < tos; s1++){
		...
		v_fill(dMin, SW_MAX, LSP_VQ_CAND);
		max_dMin = SW_MAX;
				...
				const int16_t x[], int16_t target[], int16_t max_dMin)
{
	...
	if (round_l(distortion) >= max_dMin)      /* if this situation takes place, */
...
void lsf_vq(struct melp_param *par)
{
	...
	for (i = 0; i < NF; i++){
		lsp[i] = par[i].lsf;
		lpc_lsp2pred_q(lsp[i], lpc, LPC_ORD);
		vq_lspw_q(wgt[i], lsp[i], lpc, LPC_ORD);
	}
...
void quant_fsmag(struct melp_param *par)
{
	...
	if (count > 0)
		vq_enc_q(fsvq_cb_q, par[last].fs_mag, FS_LEVELS, NUM_HARM, qmag,
			   &quant_par.fs_index);
