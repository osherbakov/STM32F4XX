			  ...
			  int16_t *pitch)
{
	...
	for (section = 0; section < BPF_ORD/2; section ++){
		iir_2nd_s(&sigbuf[BPF_ORD + PITCH_FR - FRAME],
				  &bpf_den_q[section*3], &bpf_num_q[section*3],
	...
	*pitch = frac_pch_q(&sigbuf[BPF_ORD + PITCHMAX], bpvc, fpitch[0], 5,
					  PITCHMIN, PITCHMAX, PITCHMIN_Q7, PITCHMAX_Q7,
					  MINLENGTH);

	for (i = 1; i < NUM_PITCHES; i++){
		temp = frac_pch_q(&sigbuf[BPF_ORD + PITCHMAX], &pcorr, fpitch[i], 5,
						PITCHMIN, PITCHMAX, PITCHMIN_Q7, PITCHMAX_Q7,
						MINLENGTH);
	...
	for (i = 1; i < NUM_BANDS; i++){
		...
		for (section = 0; section < BPF_ORD/2; section++){
			...
			iir_2nd_s(&sigbuf[BPF_ORD + PITCH_FR - FRAME],
					  &bpf_den_q[filt_index], &bpf_num_q[filt_index],
		...
		temp = frac_pch_q(&sigbuf[BPF_ORD + PITCHMAX], &bpvc[i], *pitch, 0,
						PITCHMIN, PITCHMAX, PITCHMIN_Q7, PITCHMAX_Q7,
						MINLENGTH);
		...
		envelope_q(&sigbuf[BPF_ORD], temp, &sigbuf[BPF_ORD], PITCH_FR);
		...
		/* Check correlations for each frame */
		temp = frac_pch_q(&sigbuf[BPF_ORD + PITCHMAX], &pcorr, *pitch, 0,
						PITCHMIN, PITCHMAX, PITCHMIN_Q7, PITCHMAX_Q7,
						MINLENGTH);
...
static int16_t temp_buf[PITCHMAX_X2];

int16_t gain_ana_q(int16_t sigin[], int16_t pitch, int16_t minlength,
				   int16_t maxlength)
{
	int16_t	length;                                                 /* Q0 */
	int16_t	flength;                                                /* Q6 */
...
int16_t lin_int_bnd_q(int16_t x, int16_t xmin, int16_t xmax,
...
void noise_est_q(int16_t gain, int16_t *noise_gain, int16_t up,
			   int16_t down, int16_t min, int16_t max)
{
	...
	temp1 = round_l(L_temp);
	L_temp = L_shl(L_deposit_l(down), 7);
	L_temp = L_add(L_noise_gain, L_temp);
	temp2 = round_l(L_temp);
...
void noise_sup_q(int16_t *gain, int16_t noise_gain, int16_t max_noise,
...
BOOLEAN q_bpvc_q(int16_t bpvc[], int16_t *bpvc_index, int16_t num_bands)
{
	register int16_t	i;
	BOOLEAN		uv_flag;
	int16_t	index;
...
void q_bpvc_dec_q(int16_t bpvc[], int16_t bpvc_index, BOOLEAN uv_flag,
				int16_t num_bands)
{
	register int16_t	i;
...
void q_gain_q(int16_t *gain, int16_t *gain_index, int16_t gn_qlo,
			...
			int16_t double_flag, int16_t scale)
{
	...
	quant_u_q(&gain[1], &gain_index[1], gn_qlo, gn_qup, gn_qlev, gn_qlev_q,
			double_flag, scale);
...
void q_gain_dec_q(int16_t *gain, int16_t *gain_index, int16_t gn_qlo,
				int16_t gn_qup, int16_t gn_qlev_q, int16_t scale)
{
	...
	quant_u_dec_q(gain_index[1], &gain[1], gn_qlo, gn_qup, gn_qlev_q, scale);
	...
	if (gain_index[0] == 0){
		...
		quant_u_dec_q(sub(gain_index[0], 1), &gain[0], temp, temp2, SIX_Q12, 3);
	}
