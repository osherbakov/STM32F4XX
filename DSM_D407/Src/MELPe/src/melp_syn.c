...
void synthesis_q(struct melp_param *par, int16_t sp_out[])
{
	...
	if (syn_begin > 0){
		if (syn_begin > par->frameSize){                             /* impossible */
			v_equ(sp_out, sigsave, par->frameSize);
			/* past end: save remainder in sigsave[0] */
			v_equ(sigsave, &sigsave[par->frameSize], (int16_t) (syn_begin - par->frameSize));
		} else
...
static void		melp_syn(struct melp_param *par, int16_t sp_out[])
{
	...
	if (firstTime){
		...
		for (i = 0; i < NUM_GAINFR; i++){
			noise_est_q(par->gain[i], &noise_gain, UPCONST_Q19, DOWNCONST_Q17,
					  MIN_NOISE_Q8, MAX_NOISE_Q8);

			/* Adjust gain based on noise level (noise suppression) */
			noise_sup_q(&par->gain[i], noise_gain, MAX_NS_SUP_Q8, MAX_NS_ATT_Q8,
					  NFACT_Q8);
		}
	}

	if (par->uv_flag && (par->rate == RATE1200)){
	...
	lpc_lsp2pred_q(par->lsf, &(lpc[1]), LPC_ORD);
	...
	lpc_gain = lpc_pred2refl_q(&(lpc[1]), sig2, LPC_ORD);
	...
	for (i = 0; i < NUM_BANDS; i++){
		if (par->bpvc[i] > X05_Q14)
			v_add(curr_pcof, bp_cof_q[i], MIX_ORD + 1);    /* bp_cof in Q14 */
		else
			v_add(curr_ncof, bp_cof_q[i], MIX_ORD + 1);
	}
...
/* Set overall interpolation path based on gain change */
		...
		interp_array_q(prev_par.lsf, par->lsf, lsf, intfact, LPC_ORD);
		lpc_lsp2pred_q(lsf, &(lpc[1]), LPC_ORD);

		/* Check signal probability for adaptive spectral enhancement filter */
		temp1 = add(noise_gain, X12_Q8);
		temp2 = add(noise_gain, X30_Q8);
		/* sig_prob in Q15 */
		sig_prob = lin_int_bnd_q(gain, temp1, temp2, 0, ONE_Q15);
		...
		interp_array_q(prev_pcof, curr_pcof, pulse_cof, temp1, MIX_ORD + 1);
		interp_array_q(prev_ncof, curr_ncof, noise_cof, temp1, MIX_ORD + 1);
		...
		v_fill(fs_real, ONE_Q13, length);
		fs_real[0] = 0;
		interp_array_q(prev_par.fs_mag, par->fs_mag, &fs_real[1], intfact,
					 NUM_HARM);
...
void melp_syn_init_q(struct melp_param *par)
{
	...
	v_fill(prev_par.fs_mag, ONE_Q13, NUM_HARM);
