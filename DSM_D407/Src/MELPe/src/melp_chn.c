...
void melp_chn_write_q(struct quant_param *qpar, unsigned char chbuf[], int32_t chwordsize)
{
	...
	pack_code_q(qpar->pitch_index, &bit_ptr, &bit_cntr, PIT_BITS, 1);
	pack_code_q(qpar->jit_index[0], &bit_ptr, &bit_cntr, 1, 1);
	pack_code_q(qpar->bpvc_index[0], &bit_ptr, &bit_cntr, NUM_BANDS - 1, 1);

	for (i = 0; i < MSVQ_STAGES; i++)
		pack_code_q(qpar->msvq_index[i], &bit_ptr, &bit_cntr, msvq_bits[i], 1);
					  ...
					  struct melp_param *prev_par, unsigned char chbuf[])
{
	...
	erase = (BOOLEAN) fec_decode_q(qpar, erase);
	...
	if (erase){                                      /* Erasure: frame repeat */
		*par = *prev_par;
		...
		v_fill(par->gain, par->gain[NUM_GAINFR - 1], NUM_GAINFR - 1);

	} else {

		/* Decode line spectrum frequencies */
		vq_msd2_q(msvq_cb_q, par->lsf, msvq_cb_mean, qpar->msvq_index,
				msvq_levels, MSVQ_STAGES, LPC_ORD, 2);
		dontcare = FS_LEVELS;
		if (qpar->uv_flag[0])
			v_fill(par->fs_mag, ONE_Q13, NUM_HARM);
		else                                     /* Decode Fourier magnitudes */
			vq_msd2_q(fsvq_cb_q, par->fs_mag, (int16_t *) NULL,
					&(qpar->fsvq_index), &dontcare, 1, NUM_HARM, 0);

		/* Decode gain terms with uniform log quantizer */
		q_gain_dec_q(par->gain, qpar->gain_index, GN_QLO_Q8, GN_QUP_Q8,
				   GN_QLEV_M1_Q10, 5);
		...
		if (qpar->uv_flag[0])
			...
			quant_u_dec_q(qpar->pitch_index, &par->pitch, PIT_QLO_Q12,
		...
		q_bpvc_dec_q(par->bpvc, qpar->bpvc_index[0], qpar->uv_flag[0],
				   NUM_BANDS);
	}
...
void low_rate_chn_write(struct quant_param *qpar, int32_t chwordsize)
{
	...
	pack_code_q(uv_index, &bit_ptr, &bit_cntr, UV_BITS, 1);

	uv_parity = parity(uv_index, 3);
	
	pack_code_q(uv_parity, &bit_ptr, &bit_cntr, 1, 1);

	pack_code_q(qpar->pitch_index, &bit_ptr, &bit_cntr, PITCH_VQ_BITS, 1);
	...
	if ((uv1 == 1) && (uv2 == 1) && (cuv == 1)){
		pack_code_q(qpar->lsf_index[0][0], &bit_ptr, &bit_cntr, 9, 1);
		pack_code_q(qpar->lsf_index[1][0], &bit_ptr, &bit_cntr, 9, 1);
		pack_code_q(qpar->lsf_index[2][0], &bit_ptr, &bit_cntr, 9, 1);
		pack_code_q(qpar->lsf_index[0][1], &bit_ptr, &bit_cntr, 4, 1);
		pack_code_q(qpar->lsf_index[1][1], &bit_ptr, &bit_cntr, 4, 1);
		pack_code_q(qpar->lsf_index[2][1], &bit_ptr, &bit_cntr, 4, 1);
		pack_code_q(lsp_prot,				 &bit_ptr, &bit_cntr, 3, 1);
	} else if ((uv1 == 1) && (uv2 == 1) && (cuv != 1)){
		pack_code_q(qpar->lsf_index[0][0], &bit_ptr, &bit_cntr, 9, 1);
		pack_code_q(qpar->lsf_index[1][0], &bit_ptr, &bit_cntr, 9, 1);
		pack_code_q(qpar->lsf_index[2][0], &bit_ptr, &bit_cntr, 8, 1);
		pack_code_q(qpar->lsf_index[2][1], &bit_ptr, &bit_cntr, 6, 1);
		pack_code_q(qpar->lsf_index[2][2], &bit_ptr, &bit_cntr, 5, 1);
		pack_code_q(qpar->lsf_index[2][3], &bit_ptr, &bit_cntr, 5, 1);
	} else if ((uv1 == 1) && (uv2 != 1) && (cuv == 1)){
		pack_code_q(qpar->lsf_index[0][0], &bit_ptr, &bit_cntr, 9, 1);
		pack_code_q(qpar->lsf_index[1][0], &bit_ptr, &bit_cntr, 8, 1);
		pack_code_q(qpar->lsf_index[1][1], &bit_ptr, &bit_cntr, 6, 1);
		pack_code_q(qpar->lsf_index[1][2], &bit_ptr, &bit_cntr, 5, 1);
		pack_code_q(qpar->lsf_index[1][3], &bit_ptr, &bit_cntr, 5, 1);
		pack_code_q(qpar->lsf_index[2][0], &bit_ptr, &bit_cntr, 9, 1);
	} else if ((uv1 != 1) && (uv2 == 1) && (cuv == 1)){
		pack_code_q(qpar->lsf_index[0][0], &bit_ptr, &bit_cntr, 8, 1);
		pack_code_q(qpar->lsf_index[0][1], &bit_ptr, &bit_cntr, 6, 1);
		pack_code_q(qpar->lsf_index[0][2], &bit_ptr, &bit_cntr, 5, 1);
		pack_code_q(qpar->lsf_index[0][3], &bit_ptr, &bit_cntr, 5, 1);
		pack_code_q(qpar->lsf_index[1][0], &bit_ptr, &bit_cntr, 9, 1);
		pack_code_q(qpar->lsf_index[2][0], &bit_ptr, &bit_cntr, 9, 1);
	} else {
		if ((uv1 != 1) && (uv2 != 1) && (cuv == 1)){
		...
		pack_code_q(qpar->lsf_index[1][0], &bit_ptr, &bit_cntr, 4, 1);

		if ((uv1 != 1) && (uv2 != 1) && (cuv == 1)){
			pack_code_q(qpar->lsf_index[2][0], &bit_ptr, &bit_cntr, 8, 1);
			pack_code_q(qpar->lsf_index[2][1], &bit_ptr, &bit_cntr, 6, 1);
			pack_code_q(qpar->lsf_index[2][2], &bit_ptr, &bit_cntr, 6, 1);
			pack_code_q(qpar->lsf_index[2][3], &bit_ptr, &bit_cntr, 6, 1);
			pack_code_q(lsp_prot,				 &bit_ptr, &bit_cntr, 3, 1);
		} else {
			pack_code_q(qpar->lsf_index[2][0], &bit_ptr, &bit_cntr, 8, 1);
			pack_code_q(qpar->lsf_index[2][1], &bit_ptr, &bit_cntr, 6, 1);
	...
	/* ====== Packing gain index ====== */
	pack_code_q(qpar->gain_index[0], &bit_ptr, &bit_cntr, GAIN_VQ_BITS, 1);

	/* ====== Packing voicing indices ====== */
	for (i = 0; i < NF; i++){
		if (!qpar->uv_flag[i])
 			pack_code_q(qpar->bpvc_index[i], &bit_ptr, &bit_cntr, 2, 1);
	}
	if (cnt == 2)
		pack_code_q(bp_prot1, &bit_ptr, &bit_cntr, 2, 1);
	else if (cnt == 1){
		pack_code_q(bp_prot2, &bit_ptr, &bit_cntr, 2, 1);
		pack_code_q(bp_prot1, &bit_ptr, &bit_cntr, 2, 1);
	} else if (cnt == 0){                                              /* UUU */
  	    pack_code_q(qpar->bpvc_index[0], &bit_ptr, &bit_cntr, 2, 1);
	    pack_code_q(bp_prot2,            &bit_ptr, &bit_cntr, 2, 1);
	    pack_code_q(bp_prot1,            &bit_ptr, &bit_cntr, 2, 1);
	}

	/* ====== Packing Fourier magnitudes ====== */
	pack_code_q(qpar->fs_index, &bit_ptr, &bit_cntr, FS_BITS, 1);
							...
							struct melp_param *prev_par)
{
	...
	if (firstTime){                                         /* Initialization */
		...
		v_fill(prev_gain, 2560, 2*NF*NUM_GAINFR);
	...
	if (flag_dec_pitch == 1){
	    if (cnt == 0){
		    ...
		    for (i = 0; i < NF; i++){
		    	if (!par[i].uv_flag)
			    	quant_u_dec_q(qpar->pitch_index, &par[i].pitch, 
	...
	for (i = 0; i < NF; i++){
		...
		q_bpvc_dec_q(&(par[i].bpvc[0]), index, qpar->uv_flag[i], NUM_BANDS);
	}

	/* ====== Decode Fourier magnitudes information ====== */
	if (cnt != 0){
		index = qpar->fs_index;
		v_equ(par[last].fs_mag, &(fsvq_cb_q[NUM_HARM*index]), NUM_HARM);
	...
	if (erase){
		for (i = 0; i < NF; i++){
			...
			v_fill(&(par[i].bpvc[0]), 0, NUM_BANDS);
			v_fill(&(par[i].fs_mag[0]), ONE_Q13, NUM_HARM);
			par[i].jitter = MAX_JITTER_Q15;
		}
		v_equ(qplsp, par[NF - 1].lsf, LPC_ORD);
		prev_uv = 1;
	    ...
	    for (i = 0; i < NF; i++){
			v_fill(&(par[i].bpvc[1]), 0, NUM_BANDS - 1);
			v_fill(&(par[i].fs_mag[0]), ONE_Q13, NUM_HARM);
		}
		prev_uv = 0;
		v_fill(prev_fsmag, ONE_Q13, NUM_HARM);
