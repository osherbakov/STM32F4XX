							   ...
							   int16_t lmin)
{
	...
	pitch = frac_pch_q(sig_in, pcorr, pitch, 0, pmin, pmax, pmin_q7, pmax_q7,
					 lmin);
	...
	for (mult = NUM_MULT; mult >= 2; mult--){
		...
		if (temp_pit >= pmin_q7){
			temp_pit = frac_pch_q(sig_in, &corr, temp_pit, 0, pmin, pmax,
			...
			if (corr > thresh){
				...
				pitch = frac_pch_q(sig_in, pcorr, temp_pit, 0, pmin, pmax,
								 pmin_q7, pmax_q7, lmin);
				break;
			}
		}
...
int16_t find_pitch_q(int16_t sig_in[], int16_t *pcorr, int16_t lower,
...
int16_t frac_pch_q(int16_t sig_in[], int16_t *pcorr, int16_t fpitch,
				   ...
				   int16_t pmin_q7, int16_t pmax_q7, int16_t lmin)
{
	...
	if (range > 0){
		...
		fpitch = shl(find_pitch_q(sig_in, &corr, lower, upper, length), 7);
	}
...
int16_t p_avg_update_q(int16_t pitch, int16_t pcorr, int16_t pthresh)
{
	register int16_t	i;
	...
	if (firstTime){
		v_fill(good_pitch, DEFAULT_PITCH_Q7, NF);
		firstTime = FALSE;
	}
					...
					int16_t *pcorr2)
{
	...
	for (section = 0; section < LPF_ORD/2; section++){
		iir_2nd_s(&sigbuf[LPF_ORD_SOS], &lpf_den_q[section*3],
		...
		iir_2nd_s(&sigbuf[LPF_ORD_SOS + FRAME], &lpf_den_q[section*3],
				  &lpf_num_q[section*3], &sigbuf[LPF_ORD_SOS + FRAME],
	...
	temp = frac_pch_q(&sigbuf[LPF_ORD_SOS + (PITCH_FR/2)], &pcorr, pitch_est,
	...
	if (pcorr < PCORR_THR){
		...
		temp = frac_pch_q(&sigbuf[LPF_ORD + (PITCH_FR/2)], &pcorr, pitch_est,
