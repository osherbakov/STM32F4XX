...
void analysis_q(int16_t sp_in[], struct melp_param *par)
{
	...
	if (par->rate == RATE2400){
		...
		vq_lspw_q(weights, par->lsf, &(lpc[1]), LPC_ORD);

		/*	msvq_enc(par->lsf, weights, par->lsf, vq_par); */
		vq_ms4_q(msvq_cb_q, par->lsf, msvq_cb_mean, msvq_levels, MSVQ_M, 4,
	...
	if (par->rate == RATE2400){
		...
		quant_u_q(&par->pitch, &(quant_par.pitch_index), PIT_QLO_Q12,
	...
	if (par->rate == RATE2400){
		...
		q_gain_q(par->gain, quant_par.gain_index, GN_QLO_Q8, GN_QUP_Q8,
			   GN_QLEV_M1, GN_QLEV_M1_Q10, 0, 5);
	} else
		gain_vq(par);

	if (par->rate == RATE2400){                                 /* Quantize jitter */
		...
		for (i = 0; i < NF; i++)
			quant_u_q(&par[i].jitter, &(quant_par.jit_index[i]), 0,
					MAX_JITTER_Q15, 2, ONE_Q15, 1, 7);
	}

	/* Quantize bandpass voicing */
	if (par->rate == RATE2400)
		par[0].uv_flag = q_bpvc_q(par[0].bpvc, &(quant_par.bpvc_index[0]), NUM_BANDS);
	else
	...
	for (i = 0; i < num_frames; i++){
		...
		/* The following v_fill() action is believed to be unnecessary. */
	...
	if (par->rate == RATE2400){
		...
		vq_enc_q(fsvq_cb_q, par->fs_mag, FS_LEVELS, NUM_HARM, par->fs_mag, &(quant_par.fsvq_index));
	} else
		quant_fsmag(par);
...
static void		melp_ana(int16_t speech[], struct melp_param *par, int16_t subnum)
{
	...
	for (section = 0; section < LPF_ORD/2; section++){
		...
		iir_2nd_s(&sigbuf[LPF_ORD], &lpf_den_q[section*3], &lpf_num_q[section*3],
				  &sigbuf[LPF_ORD], &lpfsp_delin[section*2],
				  &lpfsp_delout[section*2], FRAME);
		v_equ(&(temp_delout[2*section]), &(lpfsp_delout[2*section]), 2);
		iir_2nd_s(&sigbuf[LPF_ORD + FRAME], &lpf_den_q[section*3],
				  &lpf_num_q[section*3], &sigbuf[LPF_ORD + FRAME],
	...
	par->pitch = pitch_ana_q(&speech[FRAME_END], &sigbuf[LPF_ORD + PITCHMAX],
	...
	for (i = 0; i < NUM_GAINFR; i++){
		if (par->bpvc[0] > BPTHRESH_Q14){
			...
			temp = sub_pitch;
			par->gain[i] = gain_ana_q(&speech[FRAME_BEG + (i + 1)*GAINFR],
	...
	if (par->gain[NUM_GAINFR - 1] > SILENCE_DB_Q8)         /* par->gain in Q8 */
		...
		temp = 0;
	/* pcorr in Q14 */
	pitch_avg = p_avg_update_q(par->pitch, temp, VMIN_Q14);
...
void melp_ana_init_q(struct melp_param *par)
{
	...
	for (i = 0; i < TRACK_NUM; i++){
		v_fill(pitTrack[i].pit, FIFTY_Q0, NODE); /* !!! (12/10/99) */
		v_fill(pitTrack[i].weight, ONE_Q15, NODE);
...
void		sc_ana(struct melp_param *par)
{
	...
	for (i = 0; i < NF; i++){
		...
		if (q_bpvc_q(bpvc_copy, &index, NUM_BANDS))
			sbp[i + 1] = -1;
		else
	...
	for (i = 0; i < NF; i++){
		...
		temp1 = par[i].bpvc[0];
		q_bpvc_dec_q(par[i].bpvc, sbp[i + 1], FALSE, NUM_BANDS);
		par[i].bpvc[0] = temp1;
	}
