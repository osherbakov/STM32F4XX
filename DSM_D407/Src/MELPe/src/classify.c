...
void classify(int16_t inbuf[], classParam *classStat, int16_t autocorr[])
{
	...
	ptr_bpf_num = bpf_num_q;                   /* It will point to bpf_num[3*i] */
	ptr_bpf_den = bpf_den_q + 1;                /* and bpf_den[3*i+1]. */
	...
	v_equ(back_sigbuf, &(sigbuf_out[BPF_ORD/3 + PIT_SUBFRAME]), PIT_COR_LEN - PIT_SUBFRAME);
	...
	if (classStat->subEnergy < SILENCE_DB_Q11)
		classy = SILENCE;
			 ...
			 interp_scalar_q(voicedEn, silenceEn, X065_Q15)){
		...
		if ((lowBandCorx > X05_Q15) ||
			...
			classy = VOICED;
				 ...
				 interp_scalar_q(voicedEn, silenceEn, X03_Q15)){
		...
		if ((lowBandCorx > X055_Q15) ||
			...
			classy = VOICED;
				  ...
				  interp_scalar_q(voicedEn, silenceEn, X06_Q15)) &&
...
static void		frac_cor(int16_t inbuf[], int16_t pitch, int16_t *cor)
{
	...
	if (temp < 0) temp = 0; /* Negative Autocorrelation doesn't make sense here */
