						   ...
						   int16_t gamaK_shift[], int16_t GammaQ_TH)
{
	...
	/* Initializing qla[] */
	if (first_time){
		v_fill(qla, X05_Q15, ENH_VEC_LENF);
		first_time = FALSE;
	}
						   ...
						   int16_t bias_sub_shift[])
{
	...
	if (minspec_counter == 0){
		...
		v_fill(localflag, FALSE, ENH_VEC_LENF);
...
static void		process_frame(int16_t inspeech[], int16_t outspeech[])
{
	...
	if (first_time){
		...
		v_fill(ksi, GM_MIN, ENH_VEC_LENF);
		v_zap(ksi_shift, ENH_VEC_LENF);
		v_fill(qk, ENH_QK_MAX, ENH_VEC_LENF);
		v_fill(Gain, GM_MIN, ENH_VEC_LENF);
		v_fill(GainD, GM_MIN, ENH_VEC_LENF);
		YY_LT = 0;
		YY_LT_shift = 0;
		SN_LT0 = SN_LT;
