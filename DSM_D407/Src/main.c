...
int main(void)
{
  ...
  MX_USB_DEVICE_Init();
	...
	osParams.USB_Out_data = Queue_Create(MAX_AUDIO_SIZE_BYTES * 4, DATA_TYPE_Q15 | DATA_NUM_CH_2);
	osParams.USB_In_data = Queue_Create(MAX_AUDIO_SIZE_BYTES * 4, DATA_TYPE_Q15 | DATA_NUM_CH_2);
	
	// Queue to get the data from PDM microphone or I2S PCM data source
	osParams.PCM_In_data = Queue_Create(MAX_AUDIO_SIZE_BYTES * 4, DATA_TYPE_Q15 | DATA_NUM_CH_2);

	// Queue to pass data to the output DAC
	osParams.PCM_Out_data = Queue_Create( MAX_AUDIO_SIZE_BYTES * 4, DATA_TYPE_Q15 | DATA_NUM_CH_2);

	// Queues for Upsample and Downsample
	osParams.DownSample_data = Queue_Create( MAX_AUDIO_SIZE_BYTES * 4, DATA_TYPE_F32 | DATA_NUM_CH_1);
	osParams.UpSample_data = Queue_Create( MAX_AUDIO_SIZE_BYTES * 4, DATA_TYPE_F32 | DATA_NUM_CH_1);
