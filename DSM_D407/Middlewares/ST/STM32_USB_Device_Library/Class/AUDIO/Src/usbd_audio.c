                               ...
                               uint8_t cfgidx)
{
  ...
  else
  {
    ...
    if (((USBD_AUDIO_ItfTypeDef *)pdev->pUserData)->Init(USBD_AUDIO_FREQ) != USBD_OK)
    {
      return USBD_FAIL;
    }
    ...
    case USB_REQ_GET_INTERFACE :
			{
				...
				if (if_num == 0x01)				// Audio OUT (Speaker) Interface 
				{
					USBD_CtlSendData (pdev,
														(uint8_t *)haudio->out_alt_setting,
														1);
				}else if(if_num == 0x02)	// Audio IN (Microphone) Interface
				{
					USBD_CtlSendData (pdev,
                              ...
                              uint8_t epnum)
{
  USBD_AUDIO_HandleTypeDef   *haudio;
  haudio = (USBD_AUDIO_HandleTypeDef*) pdev->pClassData;
  if ((epnum & 0x0F) == (AUDIO_IN_EP & 0x0F))
  {
		USBD_LL_Transmit(pdev,
										 AUDIO_IN_EP,
										 &haudio->in_buffer[haudio->in_idx],
										 AUDIO_PACKET_SIZE);
		
    /* Increment the pointer and wrap it around if it goes past the buffer end */
    haudio->in_idx += AUDIO_PACKET_SIZE;
		if (haudio->in_idx >= AUDIO_TOTAL_BUF_SIZE)
    {
      haudio->in_idx = 0;
		}
                              ...
                              uint8_t epnum)
{
  ...
  if ((epnum & 0x0F) == ( AUDIO_OUT_EP & 0x0F))
  {
		uint32_t  rdy_idx;		// The index of the current ready (filled) buffer
		rdy_idx = haudio->out_idx;
    /* Increment the pointer and wrap it around if it goes beyond the buffer */
    haudio->out_idx += AUDIO_PACKET_SIZE;
    if (haudio->out_idx >= AUDIO_TOTAL_BUF_SIZE)
    {
      haudio->out_idx = 0;
		}
    ...
    USBD_LL_PrepareReceive(pdev,
                           AUDIO_OUT_EP,
                           &haudio->out_buffer[haudio->out_idx],
                           AUDIO_PACKET_SIZE);
