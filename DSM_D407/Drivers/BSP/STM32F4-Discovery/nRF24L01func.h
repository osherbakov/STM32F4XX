 ...
 * @file nRF24L01func.h
 *
 * Functions declaration for RF24 and helper enums
 */
...
s   *
  ...
  */
   ...
   *			int ok = txStandBy();     //Returns 0 if failed. 1 if success.
   ...
   *			radio.writeFast(&buf,32);
   *			radio.writeFast(&buf,32);   //Fills the FIFO buffers up
   *			int ok = txStandBy(1000);  //Returns 0 if failed after 1 second of retries. 1 if success.
  ...
  */
  void RF24_maskIRQ(int tx_ok,int tx_fail,int rx_ready);
